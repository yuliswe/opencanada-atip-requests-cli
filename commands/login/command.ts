  ...
  return new Command('login')
    .description(
      'Capture your signed-in ATIP Online session so the CLI can read your ' +
        ...
        'sign in in the browser and paste the session cookie once.'
    )
    .option(
      '--check',
    ...
    .action(async (options: { check?: boolean }) => {
      ...
      print(chalk.bold('Capture your ATIP Online session'));
      print(
      ...
      saveSession(session);
      print(
        chalk.green('Session captured and verified. ') +
          'Try "atip request sync".'
      );
    });
}
