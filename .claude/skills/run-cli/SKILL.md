...
`search`, `orgs`, the lookup step of `informal`, and all authenticated portal calls (`login`, `login --check`, `request list`, `session refresh/keepalive`) go over HTTPS. The sandbox blocks outbound network, and the symptom is a terse `fetch failed` (exit 1). Either run with `dangerouslyDisableSandbox: true`, or ask the user to run the command themselves with a `! atip ...` prompt.

Pure local subcommands (`--help`, `logout`, `request list --cached`, `request show/update/remove`, `portal`) work without network.

## Authenticated portal commands

`atip login` captures the user's ATIP Online session. By default it drives the user's own Chrome with Playwright (`playwright-core`, `channel: 'chrome'`, `launchPersistentContext` at `~/.atip-cli/chrome-profile`) and reads the HttpOnly cookie via `context.cookies()` (`utils/playwrightLogin.ts`); `--paste` is the fallback that reads a pasted `Cookie:` header via a hidden prompt. Either way the session is stored at `~/.atip-cli/session.json` (mode 0600). `request list` calls the portal's `GetMyRequestsList` endpoint with that cookie, write-through caches the results into the tracker, and renders it; `request list --cached` renders the tracker without contacting the portal.
...
- `GetMyRequestsList` returns a rendered HTML `<table>` fragment, not JSON; `parseRequestListRows` reads `<tr class="request_row">` cells. `request list --json` prints the parsed objects; `request list --raw` dumps the raw HTML for re-mapping.
